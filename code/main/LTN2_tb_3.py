# improved by Fujian Yan from LTN library.
# contact: fujian.yan@wichita.edu
...
# def addlayer(label,number_of_features_or_vars,tensor):
#
#     result1 = tf.keras.layers.Dense(units=300,activation='relu',name='Dense')(tensor)
#     result2 = tf.keras.layers.Dense(units=150,activation='relu',name='Dense2')(result1)
#     # result3 = tf.keras.layers.Dense(units=75,activation='relu',name='Dense3')(result2)
#     # result4 = tf.keras.layers.Dense(units=150,activation='relu',name='Dense4')(result3)
#     result5 = tf.keras.layers.Dense(units=300,activation='relu',name='Dense5')(result2)
#     result = tf.keras.layers.Dense(units=number_of_features_or_vars,activation='relu',name='Dense6')(result5)
#     # result = result3
#     result.doms = [label]
#     return result
def addlayer(label,number_of_features_or_vars,tensor):

    result1 = tf.keras.layers.Dense(units=500,activation='relu',name='Dense')(tensor)
    result2 = tf.keras.layers.Dense(units=400,activation='relu',name='Dense2')(result1)
    result3 = tf.keras.layers.Dense(units=300,activation='relu',name='Dense3')(result2)
    result4 = tf.keras.layers.Dense(units=200,activation='relu',name='Dense4')(result3)
    result5 = tf.keras.layers.Dense(units=100,activation='relu',name='Dense5')(result4)
    result = tf.keras.layers.Dense(units=number_of_features_or_vars,activation='relu',name='Dense6')(result5)
    # result = result3
    result.doms = [label]
    return result
def addlayer_xyz(label,number_of_features,tensor):
    result1 = tf.keras.layers.Dense(units = 150, activation='relu',name = 'Dense')(tensor)
    # result2 = tf.keras.layers.Dense(units = 75, activation='relu',name = 'Dense2')(result1)
    # result3 = tf.keras.layers.Dense(units = 150, activation='relu', name = 'Dense3')(result2)
    result4 = tf.keras.layers.Dense(units = number_of_features, activation = 'relu', name = 'Dense4')(result1)
    result4.doms=[label]
    return result4

def predicate(label,number_of_features_or_vars,pred_definition=None,layer=None):
    ...
    if pred_definition is None:
        ...
        def apply_pred(*args):
            ...
            X = tf.concat([tf.ones((tf.shape(tensor_args)[0], 1)),
                           tensor_args], 1)
            XW = tf.matmul(tf.tile(tf.expand_dims(X, 0), [layer, 1, 1]), W)
